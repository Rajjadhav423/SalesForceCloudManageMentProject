...
import { Button } from '../ui/button';
import { Switch } from '../ui/switch';
import { Loader2, Search, RefreshCw, Info, ArrowRight } from 'lucide-react';
import { Badge } from '../ui/badge';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '../ui/tabs';
import { Label } from '../ui/label';
import { Skeleton } from '../ui/skeleton';
...
// Custom node component with fields preview
const SalesforceObjectNode = ({ data }) => {
  return (
    <div 
      className={`px-4 py-2 rounded-md shadow-md w-64 border-2 ${
        data.custom ? 'border-blue-400 bg-blue-50' : 'border-gray-400 bg-white'
      }`}
    >
      <div className="flex items-center justify-between">
        <div className="font-bold text-sm truncate">{data.label}</div>
      ...
      {/* Display key fields */}
      {data.keyFields && data.keyFields.length > 0 && (
        <div className="mt-2 border-t border-dashed border-gray-300 pt-1">
          <div className="text-xs font-medium text-gray-600">Key Fields:</div>
          <div className="text-xs space-y-1 mt-1">
            {data.keyFields.map((field, idx) => (
              <div key={idx} className="flex items-center">
                <div className="w-2 h-2 bg-gray-400 rounded-full mr-1"></div>
                <span className="truncate" title={`${field.label} (${field.name})`}>
                  {field.label}
                </span>
              </div>
            ))}
      ...
      <div className="mt-2 text-xs flex space-x-2">
        <Badge variant="outline" className="flex items-center">
          <span className="mr-1">{data.fieldCount}</span> Fields
        </Badge>
        {data.relationshipCount > 0 && (
          <Badge variant="outline" className="flex items-center">
            <span className="mr-1">{data.relationshipCount}</span> Relations
          </Badge>
        )}
      </div>
    </div>
...
// Custom edge with tooltip showing relationship details
const SalesforceRelationshipEdge = ({ data, ...props }) => {
  return (
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <div>
            <g>
              {/* Your edge will be rendered here by ReactFlow */}
              {props.children}
            </g>
          </div>
        </TooltipTrigger>
        <TooltipContent className="max-w-xs">
          <div className="text-xs">
            <div className="font-bold">{data.fieldName}</div>
            {data.relationshipName && (
              <div>Relationship: {data.relationshipName}</div>
            )}
            {data.required && <Badge variant="outline">Required</Badge>}
          </div>
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
...
// Function to create layout with Dagre
const getLayoutedElements = (nodes, edges, direction = 'TB') => {
  const dagreGraph = new dagre.graphlib.Graph();
  dagreGraph.setDefaultEdgeLabel(() => ({}));
...
const SchemaBuilder = () => {
  ...
  const nodeTypes = { salesforceObject: SalesforceObjectNode };
  const edgeTypes = { salesforceRelationship: SalesforceRelationshipEdge };

  // Fetch schema data - only once
  useEffect(() => {
    const fetchSchema = async () => {
      try {
        const savedAuthData = localStorage.getItem('sfAuthData');
        ...
        field.referenceTo.forEach(targetObjName => {
          ...
          if (objectMap[targetObjName]) {
            const edge = {
              ...
              source: obj.name,
              target: targetObjName,
              type: 'salesforceRelationship',
              animated: false,
              label: field.relationshipName || field.name,
              labelStyle: { fill: '#666', fontWeight: 300, fontSize: 12 },
              style: { stroke: '#999', strokeWidth: 1.5 },
              markerEnd: {
          ...
          <div className="space-y-4">
            ...
            <div className="pt-4 border-t">
              <h3 className="text-sm font-medium mb-2 flex items-center">
                <Info className="h-4 w-4 mr-1" /> Usage Tips
              </h3>
              <div className="text-xs text-gray-600 space-y-2">
                <div className="flex items-start">
                  <ArrowRight className="h-3 w-3 mr-1 mt-0.5 flex-shrink-0" />
                  <span>Click on objects to see details</span>
                </div>
                <div className="flex items-start">
                  <ArrowRight className="h-3 w-3 mr-1 mt-0.5 flex-shrink-0" />
                  <span>Hover over relationship lines to see field details</span>
                </div>
                <div className="flex items-start">
                  <ArrowRight className="h-3 w-3 mr-1 mt-0.5 flex-shrink-0" />
                  <span>Use the search box to filter objects</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="p-4">
              ...
              <Tabs defaultValue="fields">
                ...
                <TabsContent value="fields" className="mt-2">
                  ...
                  />
                  <div className="space-y-1 max-h-96 overflow-y-auto">
                    {getFilteredFields(selectedObject.fields).map(field => (
                      <div 
                        key={field.name}
                        className={`p-2 rounded-md cursor-pointer hover:bg-slate-100 ${
                          selectedField?.name === field.name ? 'bg-slate-100' : ''
                      ...
                      >
                        ...
                        <div className="flex space-x-1 mt-1">
                          ...
                          {field.nameField && (
                            <Badge variant="outline" className="text-xs">Name Field</Badge>
                          )}
                        </div>
                      </div>
                    ))}
                  ...
                  {selectedObject.fields
                    ...
                    .map(field => (
                      <div key={field.name} className="p-2 mb-2 border rounded-md">
                        ...
                        <div className="text-xs text-gray-500 mb-1">{field.name}</div>
                        <div className="text-xs mt-2">References:</div>
                        <div className="flex flex-wrap gap-1 mt-1">
                          {field.referenceTo.map(refObj => (
                            <Badge key={refObj} variant="secondary" className="text-xs">
                              {refObj}
              ...
              {selectedField && (
                <div className="mt-4 border-t pt-4">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div className="font-medium">Required:</div>
                    <div>{selectedField.required ? 'Yes' : 'No'}</div>
                    {selectedField.unique !== undefined && (
                      <>
                        <div className="font-medium">Unique:</div>
                        <div>{selectedField.unique ? 'Yes' : 'No'}</div>
                      </>
                    )}
                    {selectedField.externalId !== undefined && (
                      <>
                        <div className="font-medium">External ID:</div>
                        <div>{selectedField.externalId ? 'Yes' : 'No'}</div>
                      </>
                    )}
                    {selectedField.nameField !== undefined && (
                      <>
                        <div className="font-medium">Name Field:</div>
                        <div>{selectedField.nameField ? 'Yes' : 'No'}</div>
                      </>
                    )}
                    {selectedField.referenceTo && selectedField.referenceTo.length > 0 && (
                      <>
