...
const AIQueryAssistant = () => {
  ...
  const exportToCSV = () => {
    ...
    try {
      ...
      link.setAttribute('download', `salesforce_query_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const renderQueryResults = () => {
    ...
    return (
      <ScrollArea className="h-[400px] w-full rounded-md border border-slate-200 dark:border-slate-700">
        <div className="w-full p-0">
          <table className="w-full border-collapse">
            <thead className="sticky top-0 bg-slate-50 dark:bg-slate-800">
              <tr>
                {fields.map(field => (
                  <th key={field} className="p-3 text-left border-b border-gray-200 dark:border-gray-700 font-semibold text-sm text-gray-700 dark:text-gray-300">
                    {field}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {queryResults.map((record, index) => (
                <tr key={index} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  {fields.map(field => (
                    <td key={`${index}-${field}`} className="p-3 border-b border-gray-100 dark:border-gray-800 text-sm dark:text-gray-300">
                      {typeof record[field] === 'object' ? 
                        JSON.stringify(record[field]) : String(record[field] || '')}
                    </td>
                  ))}
  ...
  // Connection status
  const connectionStatus = token && instanceUrl ? (
    <Alert className="mb-4 bg-green-50 dark:bg-green-900/20 text-green-800 dark:text-green-300 border-green-100 dark:border-green-900/50">
      <Database className="h-4 w-4 text-green-600 dark:text-green-400" />
      <AlertTitle className="text-green-800 dark:text-green-300">Connected to Salesforce</AlertTitle>
      <AlertDescription className="text-green-700 dark:text-green-400 text-xs">
        Instance: {instanceUrl.replace('https://', '')}
        <Button 
          variant="link" 
          size="sm" 
          className="text-xs p-0 h-4 ml-2 text-green-800 dark:text-green-300" 
          onClick={() => setShowConnectionStatus(false)}
        >
          Dismiss
        </Button>
    ...
    <Alert className="mb-4 bg-amber-50 dark:bg-amber-900/20 text-amber-800 dark:text-amber-300 border-amber-100 dark:border-amber-900/50">
      <AlertCircle className="h-4 w-4 text-amber-600 dark:text-amber-400" />
      <AlertTitle className="text-amber-800 dark:text-amber-300">Not connected to Salesforce</AlertTitle>
  ...
  return (
    <Card className="w-full shadow-lg border-slate-200 dark:border-slate-700 dark:bg-slate-900 transition-colors">
      ...
      <CardContent className="p-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid grid-cols-4 mb-6 bg-slate-100 dark:bg-slate-800 p-1">
            {Object.entries(tabIcons).map(([key, icon]) => (
              <TabsTrigger 
                key={key} 
                value={key}
          ...
          <TabsContent value="generate" className="space-y-4 mt-2">
            <div className="space-y-2">
              ...
              <Textarea
                ...
                value={naturalLanguagePrompt}
                onChange={(e) => setNaturalLanguagePrompt(e.target.value)}
                className="min-h-[150px] text-base border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300"
              />
            ...
            <div className="flex justify-end mt-6">
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent className="bg-slate-800 text-white dark:bg-slate-700">
                    <p>Transform your natural language into a SOQL query</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            ...
            <div className="space-y-2">
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button 
                          ...
                          size="sm" 
                          onClick={copyQuery} 
                          className="gap-1 border-slate-200 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
                        >
                          <Copy className="h-3.5 w-3.5" />
                          Copy
                        </Button>
              ...
              <div className="relative">
                <Textarea
                  value={generatedQuery}
                  onChange={(e) => setGeneratedQuery(e.target.value)}
                  className="font-mono text-sm min-h-[200px] bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 dark:text-slate-300"
                />
                {!generatedQuery && (
          ...
          <TabsContent value="results" className="space-y-4 mt-2">
            <div className="flex justify-between items-center">
              <h3 className="text-sm font-medium flex items-center gap-2 text-slate-700 dark:text-slate-300">
                <Database className="h-4 w-4 text-blue-500" />
                Query Results
                {queryResults.length > 0 && (
                  <Badge variant="secondary" className="ml-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300">
                    {queryResults.length} records
                  </Badge>
                )}
              </h3>
                ...
                <div className="flex gap-2">
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent className="bg-slate-800 text-white dark:bg-slate-700">
                        <p>Download query results as CSV file</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </div>
            ...
            {error ? (
              <Alert variant="destructive" className="bg-red-50 dark:bg-red-900/20 text-red-800 dark:text-red-300 border-red-200 dark:border-red-900/50">
                <AlertCircle className="h-4 w-4" />
                <AlertTitle>Error executing query</AlertTitle>
                <AlertDescription className="font-mono text-sm mt-2">
                  {error}
            ...
            {queryHistory.length > 0 ? (
              <div className="space-y-3">
                {queryHistory.map((item) => (
                  ...
                  >
                    <Card className="overflow-hidden border-slate-200 dark:border-slate-700 hover:border-blue-200 dark:hover:border-blue-800 transition-colors dark:bg-slate-800">
                      <CardContent className="p-4">
                        <div className="font-medium text-blue-700 dark:text-blue-400">{item.prompt}</div>
                        <div className="text-xs text-gray-500 dark:text-gray-400 font-mono mt-2 line-clamp-1 bg-slate-50 dark:bg-slate-900 p-2 rounded">
                          {item.query}
                        </div>
                        <div className="flex justify-between items-center mt-3">
                          <div className="flex items-center gap-2">
                          ...
                          <Button 
                            ...
                            size="sm" 
                            onClick={() => loadQueryFromHistory(item)}
                            className="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 gap-1"
                          >
                            Use Again
                            <ChevronRight className="h-3.5 w-3.5" />
                          </Button>
      ...
      {error && (
        <CardFooter className="bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300 p-4 text-sm flex items-start gap-2 border-t border-red-100 dark:border-red-900/50">
          <AlertCircle className="h-5 w-5 flex-shrink-0 mt-0.5" />
          <div>
            <strong className="font-medium">Error:</strong> {error}
          </div>
      ...
      <CardFooter className="border-t border-slate-200 dark:border-slate-700 p-4 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50 text-sm text-slate-500 dark:text-slate-400">
        <div className="flex items-center gap-1">
          <Info className="h-4 w-4" />
          <span>AI-powered SOQL Query Builder</span>
        </div>
